diff -u -rN --exclude=CVS --exclude=*.pyc --exclude=*~ --exclude=.#* TwistedClean/twisted/internet/apploader.py Twisted/twisted/internet/apploader.py
--- TwistedClean/twisted/internet/apploader.py	Wed Dec 31 19:00:00 1969
+++ Twisted/twisted/internet/apploader.py	Wed Nov 13 11:21:01 2002
@@ -0,0 +1,155 @@
...
+           }
+
+def loadPersisted(filename, kind='guess', encrypted=0, passphrase=''):
+    "Loads filename, of the specified kind and returns an application"
+    Loader = loaders[kind]
+    l = Loader(filename, encrypted, passphrase)
+    application = l.load()
+    return application
diff -u -rN --exclude=CVS --exclude=*.pyc --exclude=*~ --exclude=.#* TwistedClean/twisted/scripts/coil.py Twisted/twisted/scripts/coil.py
--- TwistedClean/twisted/scripts/coil.py	Wed Apr 24 01:58:21 2002
+++ Twisted/twisted/scripts/coil.py	Mon Nov 25 17:35:55 2002
@@ -37,8 +37,9 @@
...
+        application = loadPersisted(tapFile, config['format'])
+        #f = open(tapFile, "rb")
+        #application = pickle.loads(f.read())
+        #f.close()
...
diff -u -rN --exclude=CVS --exclude=*.pyc --exclude=*~ --exclude=.#* TwistedClean/twisted/scripts/mktap.py Twisted/twisted/scripts/mktap.py
--- TwistedClean/twisted/scripts/mktap.py	Mon Sep 30 04:25:20 2002
+++ Twisted/twisted/scripts/mktap.py	Thu Nov 14 12:54:23 2002
@@ -70,14 +70,21 @@
...
+    from twisted.internet.apploader import guess, loadPersisted
+    filename = options['append']
+    a = loadPersisted(filename, options['type'],
+                      options['encrypted'], passphrase)
+    if os.path.splitext(filename)[1] == '.py':
+        options['append'] = filename[:-3] + '.tas'
+
...
+    if options['append']:
+        try:
+            loadForAppend(options, passphrase)
+        except:
+            options['append'] = None
...
diff -u -rN --exclude=CVS --exclude=*.pyc --exclude=*~ --exclude=.#* TwistedClean/twisted/scripts/tapconvert.py Twisted/twisted/scripts/tapconvert.py
--- TwistedClean/twisted/scripts/tapconvert.py	Tue Nov 26 00:07:32 2002
+++ Twisted/twisted/scripts/tapconvert.py	Tue Nov 26 00:05:31 2002
@@ -22,122 +22,7 @@
 from cPickle import load
 from cStringIO import StringIO
...
diff -u -rN --exclude=CVS --exclude=*.pyc --exclude=*~ --exclude=.#* TwistedClean/twisted/scripts/twistd.py Twisted/twisted/scripts/twistd.py
--- TwistedClean/twisted/scripts/twistd.py	Tue Nov 26 00:07:33 2002
+++ Twisted/twisted/scripts/twistd.py	Tue Nov 26 00:01:40 2002
@@ -20,6 +20,7 @@
 from twisted.python import usage, util, runtime, register, plugin
 from twisted.python import log, logfile
 from twisted.persisted import styles
+from twisted.internet.apploader import loaders, loadPersisted
 util.addPluginDir()
